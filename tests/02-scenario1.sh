...
NPROC=$(cat tests/02-scenario1.txt | head -1)
...
rm -fr tmp/02scenario1 > /dev/null
mkdir tmp/02scenario1 -p

WORKDIR=`pwd`/tmp/02scenario1
PROCS=""
...
LICZNIK=0
for i in `cat tests/02-scenario1.txt | awk '{ if (NR!=1) { print $0}}'`;
do
	LICZNIK=$(($LICZNIK+1))
